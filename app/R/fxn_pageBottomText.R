...
fxn_pageBottomText <- function() {
  ...
  temperatureHumidityIndexURL <- 
    a(
      "equation", 
      href="https://www.nationalacademies.org/publications/20608",
      target="_blank"
    )
  ...
  pageBottomText <- 
    htmltools::p(
      htmltools::HTML(
        paste0(
          "Sparkline charts show data over the past 12 hours. Heat index is the apparent or ", '"', "feels like", '"', " temperature based on air temperature, relative humidity, and the National Weather Service ", heatIndexEquationURL, ". THI is the Temperature Humidity Index for heat stress in cattle based on air temperature, relative humidity, and the National Research Council (1971) ", temperatureHumidityIndexURL, ". Maximum values, minimum values, and daily totals are since midnight local time. Values of 'NA' denote no data.",
